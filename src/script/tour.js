...
const BASE_URL = "https://oystein.taskjelle.no/tour2023-prod/";
...
let createPenaltyMatrix = function( hosts, _groupMeals, meals, _groupId = -1, meal = 0 ) {
let matrix = {};
let hostMatrix = {};
let emptyList = {};
...
let createPenaltyMatrix2 = function( _groupMeals, _meals, _groupId = -1, _meal = 0 ) {
let matrix = {};
let hostMatrix = {};
...
let createSchedulePenalty = function( hosts, groups, groupMeals, meals, clearAll = false ) {
if( clearAll ) {
...
for( ; o < numberOfIter; o++ ) {
   ...
   for( let i = 0; i < MEALS.length; i++ ) {
       freeSeatsPerHost_running[MEALS[i]] = freeSeats( hosts, groups, MEALS, -1, true, i );
   }
   ...
   for( let j = 0; j < MEALS.length; j++ ) {
       for( let i = 0; i < groupsSorted.length; i++ ) {
           ...
           let unVisitedHosts = hostKeys.filter( x => groupmeals_running[groupId].indexOf( x ) == -1 ); // not visited hosts
           ...
           let penaltyMatrix = createPenaltyMatrix2( groupmeals_running, MEALS, groupId, j);
           ...
           */
           ...
           freeSeatsPerHost_running[MEALS[j]][chosenHost] = freeSeatsPerHost_running[MEALS[j]][chosenHost] - countInGroup;
   ...
   if( o == 0 ) {
       console.log(o + ": " + missing_running.length + " missing and " + maxPenalty_r + " max penalty. 0 (chosen).");
       ...
       console.log(o + ": " + missing_chosen.length + " -> " + missing_running.length + " missing and " + maxPenalty_c + " -> " + maxPenalty_r + " max penalty. 1 (chosen).");
       missing_chosen = missing_running.splice(0);
       groupmeals_chosen = Object.assign({}, groupmeals_running);
       maxPenalty_c = maxPenalty_r;
   }
   else if( missing_running.length == missing_chosen.length && maxPenalty_r < maxPenalty_c ) {
       console.log(o + ": " + missing_chosen.length + " -> " + missing_running.length + " missing and " + maxPenalty_c + " -> " + maxPenalty_r + " max penalty. 2 (chosen).");
       missing_chosen = missing_running.splice(0);
       groupmeals_chosen = Object.assign({}, groupmeals_running);
       maxPenalty_c = maxPenalty_r;
   }
...
let freeSeats = function( hosts, groups, meals, hostId = -1, perHost = true, meal = -1 ) {
if( meal == -1 ) {
   console.debug("Method: freeSeats. Error. Input argument 'meal' is undefined.");
   return undefined;
}
else if( hostId < 0 && !perHost ) { // return total number of free seats
   return countOfSeats( hosts, hostId ) -  countOfGuests( groups, -1 );
}
else if( hostId < 0 && perHost ) { // return array of count of free seats per host
   let result = {};
   for( let _hostId in hosts ) {
...
else if( hostId != -1 ) { // return the number of free seats of host at a meal
...
for( let i = 0; i < MEALS.length; i++ ) {
   let meal = MEALS[i];
   $(div).append("<div class='element-meal-" + meal + "' data-meal-id='" + meal + "'></div>");
   ...
   for( host in hosts ) {
       ...
       $(".element-meal-" + meal).append("<div class='element-host-" + host + "' data-host-id='" + host + "'></div>");
       $(".element-meal-" + meal + " .element-host-" + host).append("<div class='heading'></div>");
       $(".element-meal-" + meal + " .element-host-" + host + " .heading").append("Host: " + hosts[host].name + " (" + freeSeatsAtMeal + ")");  

       for( let j = 0; j < hosts[host][meal].length; j++ ) {
           let groupId = hosts[host][meal][j];
           $("<div class='element-group-" + groupId + "' data-group-id='" + groupId + "'></div>").appendTo(".element-meal-" + meal + " .element-host-" + host)
               .mouseenter(function() {
           ...
           for( let k = 0; k < groups[groupId].length; k++ ) {
               $(".element-meal-" + meal + " .element-host-" + host + " .element-group-" + groupId ).append( groups[groupId][k].firstname + " " + groups[groupId][k].lastname + "<br/>" );
           }
       }

       for( let j = 0; j < freeSeatsAtMeal; j++ ) {
           $("<div class='element-empty'>&nbsp;</div>").appendTo(".element-meal-" + meal + " .element-host-" + host);
       }
   }
}   
}
...
for( group in groupMeals ) {
   let groupId = group;
   let title = "";
   $( div ).append("<div class='group-meals' id='group-" + groupId + "-meals'></div>");
   $( "<div class='group-guests element-group-" + groupId + "' data-group-id='" + groupId + "' id='group-" + groupId + "-guests' draggable='true'></div>").appendTo( div )
       .mouseenter(function() {
           $( ".element-group-" + groupId ).toggleClass( "highlight", true);
       })
       .mouseout(function() {
   ...
   for( let i = 0; i < groups[groupId].length; i++ ) {
       $( "#group-" + groupId + "-guests" ).append( groups[groupId][i].firstname + " " + groups[groupId][i].lastname );
   ...
   for( let i = 0; i < groupMeals[groupId].length; i++ ) {
       $( "#group-" + groupId + "-meals" ).append( (groupMeals[groupId][i] < 0 ) ? "&#11046;":"&#10003;" );
   }
}
}
...
if( ev.target.parentNode.attributes.hasOwnProperty( "data-host-id" )) {
   visitArray[0] = ev.target.parentNode.parentNode.attributes.getNamedItem("data-meal-id").value;
...
if( this.id == "groups" ) {
   ...
   let newVisitArray = [];
   newVisitArray[0] = e.target.parentNode.parentNode.attributes.getNamedItem("data-meal-id").value;
   newVisitArray[1] = e.target.parentNode.attributes.getNamedItem("data-host-id").value;
   let oldVisitArray = JSON.parse( e.dataTransfer.getData( 'text/plain' ));

   moveVisit(hosts, groupMeals, MEALS, newVisitArray[0], newVisitArray[1], oldVisitArray[1] );
   drawGroups( "#groups", groups, groupMeals );
   drawSchedule( "#schedule", hosts, groups, MEALS );
   addDragListeners();
}
...
document.querySelector("#menu-grouping").addEventListener("click", function( e ) {
...
document.querySelector("#menu-clear-schedule").addEventListener("click", function( e ) {
...
httpRequest.open( 'POST', BASE_URL + 'save.php' );
